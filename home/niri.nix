...
in
{
  xdg.configFile."niri/config.kdl".text = ''
    ...
    binds {
        ...
        "Super+Alt+L" { spawn "sh" "-c" "noctalia msg session lock 2>/dev/null || swaylock"; }
        "Mod+Shift+S" { spawn "sh" "-c" "grim -g \"$(slurp)\" - | satty --filename - --fullscreen --output-filename ~/Pictures/Screenshots/satty-$(date +%Y-%m-%d_%H-%M-%S).png"; }
        "Print" { spawn "sh" "-c" "grim - | satty --filename - --fullscreen --output-filename ~/Pictures/Screenshots/satty-$(date +%Y-%m-%d_%H-%M-%S).png"; }
